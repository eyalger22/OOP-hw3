...
import src.BusinessLayer.Enemies.Monster;
import src.BusinessLayer.Enemies.Trap;
import src.BusinessLayer.Enemies.Boss;
...
public class TileFactory {
    ...
    public Enemy produceEnemy(char c, Position position, CLI cli, Board board) {
        try{
            Enemy e = enemiesMap.get(c).get();
            e.initialize(position);
            e.initCallbacks((s)->cli.printMessage(s),()->board.remove(e), (pos)->board.getTile(pos));
            return e;
        }
        catch (Exception e){
            return null;
    ...
    public Player producePlayer(int idx, Position position, CLI cli, Board board){
        Player p = playersList.get(idx).get();
        p.initialize(position);
        p.initCallbacks((s)->cli.printMessage(s),()->cli.playerDeath(p), (pos)->board.getTile(pos));
        return p;
    }
